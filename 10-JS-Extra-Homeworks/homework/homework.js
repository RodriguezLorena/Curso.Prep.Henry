...
// const object1 = string;

// const object2 = Object.fromEntries(
//   Object.entries(object1).map(([ key, val ]) => [ key, val === val ])
// );
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  let arrayDePalabras= str.split(" ");
  for (let i = 0; i < arrayDePalabras.length; i++) {
    arrayDePalabras[i] = arrayDePalabras[i].split("").reverse().join("")
...
//   return "No es capicua"
  ...
  let numCap = numero.toString().split("").reverse().join("")
  if(numCap === numero.toString()){
     return  "Es capicua"
}
  return "No es capicua"
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
 let indices= cadena.split("");
 let nuevaCadena=[];
    for(let i= 0; i < indices.length; i++){
   
      if(indices[i] !== "a" && indices[i] !=="b" && indices[i] !== "c"){
       nuevaCadena.push(indices[i])
